...
class DictLoader:
    """PyTorch DataLoader for :class:`~biobb_pytorch.mdae.data.dataset.DictDataset` .
    ...
    A ``DictLoader`` can be initialize from a ``dict``, a :class:`~biobb_pytorch.mdae.data.dataset.DictDataset`,
    or a ``Subset`` wrapping a :class:`~biobb_pytorch.mdae.data.dataset.DictDataset`.
    ...
    def set_dataset_and_batch_size(
        ...
        if self.has_multiple_datasets:
            ...
            if len(self._batch_size) != len(self._dataset):
                batch_size_len = len(self._batch_size)
